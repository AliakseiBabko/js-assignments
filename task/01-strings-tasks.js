...
function encodeToRot13(str) {
    let inputAlphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    let outputAlphabet = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    let len = str.length, result = '';
    for (let i=0; i<len; i++) {
        result += outputAlphabet[inputAlphabet.indexOf(str[i])] || str[i];
    }
    return result;
}
...
function getCardId(value) {
    let result = 0, firstValue = value.slice(0,1), secondValue = value.slice(1,2);
    switch (firstValue) {
        case 'A':
        result += 0;
        case 'J':
